...
load('lorenz_train_data_dt_0_01_nst_100_T_10.mat')
...
% 
% save('Lorenz_trained_network_dlnet.mat', 'dlnet')
% save('Lorenz_train_val_loss_dlnet.mat', 'trainLoss', 'valLoss', 'num_iters')

% Using a trained model
load("Lorenz_trained_network_dlnet.mat")
load("Lorenz_train_val_loss_dlnet.mat")
...
for k = 1:num_samples
    ...
    y = C*test_targets(k,:)' + sqrt(measurement_var)*randn*1;    % Measurement
    y_tot(k) = y;
...
figure
plot3(test_inputs(1, 1), test_inputs(1, 2), test_inputs(1, 3), 'ro', 'LineWidth',2)
hold on
plot3(test_targets(:,1), test_targets(:,2), test_targets(:,3),'k')
plot3(x_est_store(1,:), x_est_store(2,:), x_est_store(3,:),'b--')
grid on
view(15,20)
% x_est_store_w_EKF = x_est_store;
% x_est_store_wo_EKF = x_est_store; 
% save('x_est_store_w_EKF_file.mat', 'x_est_store_w_EKF')
% save('x_est_store_wo_EKF_file.mat', 'x_est_store_wo_EKF')


figure
plot3(test_targets(1, 1), test_targets(1, 2), test_targets(1, 3), 'ro', 'LineWidth',2)
hold on
plot3(test_targets(:,1), test_targets(:,2), test_targets(:,3),'k')
grid on
view(15,20)
...
ylim([-30 30])
zlim([0 60])
% print(gcf,'-dpng','Figures/Lorenz/png/Lorenz_dt_0_01_T_30_traj')
% print(gcf,'-depsc','Figures/Lorenz/eps/Lorenz_dt_0_01_T_30_traj')


figure
plot3(x_est_store(1, 1), x_est_store(2,1), x_est_store(3,1), 'ro', 'LineWidth',2)
hold on
plot3(x_est_store(1,:), x_est_store(2,:), x_est_store(3,:),'b')
grid on
view(15,20)
...
ylim([-30 30])
zlim([0 60])
% print(gcf,'-dpng','Figures/Lorenz/png/Lorenz_NN_EKF_dt_0_01_T_30_traj')
% print(gcf,'-depsc','Figures/Lorenz/eps/Lorenz_NN_EKF_dt_0_01_T_30_traj')
% print(gcf,'-dpng','Figures/Lorenz/png/Lorenz_NN_dt_0_01_T_30_traj')
% print(gcf,'-depsc','Figures/Lorenz/eps/Lorenz_NN_dt_0_01_T_30_traj')

%%% If you remove the close all you can plot the true, NN and NN_EKF
%%% trajectories on the same pot (you also need to give the same numebr to the figure functions, i.e. figure(5))

% print(gcf,'-dpng','Figures/Lorenz/png/Lorenz_NN_NNEKF_dt_0_01_T_30_traj')
% print(gcf,'-depsc','Figures/Lorenz/eps/Lorenz_NN_NNEKF_dt_0_01_T_30_traj')
...
plot(test_targets(:,1),'g', LineWidth=2)
hold on
plot(x_est_store(1,:),'r', LineWidth=2)
set(gca,'fontsize',16);
set(gca,'xticklabel',{[]})
ylabel('$x_1$', Interpreter='latex', fontsize=22)
grid on 
...
plot(test_targets(:,2),'g', LineWidth=2)
hold on
plot(x_est_store(2,:),'r', LineWidth=2)
set(gca,'fontsize',16);
set(gca,'xticklabel',{[]})
ylabel('$x_2$', Interpreter='latex', fontsize=22)
grid on
...
plot(test_targets(:,3),'g', LineWidth=2)
hold on
plot(x_est_store(3,:),'r', LineWidth=2)
set(gca,'fontsize',16);
set(gca,'xticklabel',{[]})
ylabel('$x_3$', Interpreter='latex', fontsize=22)
grid on
...
% print(gcf,'-dpng','Figures/Lorenz/png/Lorenz_NN_NNEKF')
% print(gcf,'-depsc','Figures/Lorenz/eps/Lorenz_NN_NNEKF')
